...
def main():
    ...
    parser.add_argument('--target-column', type=str, default=None, help="Target column name. Overrides auto schema discovery.")
    parser.add_argument('--problem-type', type=str, default=None, choices=['classification', 'regression', 'anomaly_detection'], help="Problem type. Overrides auto schema discovery.")
    parser.add_argument('--feature-columns', type=str, default=None, help="Comma-separated feature column names. Default: all except target/IDs/timestamps.")
    ...
    def auto_select_from_schema(csv_path: str):
        ...
        # User overrides take precedence over schema discovery
        if args.target_column or args.problem_type:
            ...
            # Fill any missing piece via schema discovery
            if not target or not problem:
                schema = discover_dataset_schema(df)
                suggested = schema.get('suggested_targets', []) or []
                target_match = next((s for s in suggested if s.get('column') == target), None) if target else None
                if target_match:
                    problem = problem or target_match.get('suggested_task', 'classification')
                elif suggested:
                    best = sorted(suggested, key=lambda x: x.get('score', 0), reverse=True)[0]
                    target = target or best.get('column')
                    problem = problem or best.get('suggested_task', 'classification')
                if target and not problem:
                    col = df[target]
                    problem = 'classification' if (str(col.dtype) in ['object', 'category', 'bool'] or col.nunique() <= 20) else 'regression'
            ...
            logging.info(f"[USER OVERRIDE] target={target}, problem={problem}, features={feature_cols}")
    ...
    def run_single_dataset(csv_path: str):
        # If user specified target/problem, treat as auto mode regardless of --auto flag
        user_specified = bool(args.target_column or args.problem_type)
        if args.auto or user_specified:
